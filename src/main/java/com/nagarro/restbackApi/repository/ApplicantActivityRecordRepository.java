...
public interface ApplicantActivityRecordRepository extends JpaRepository<ApplicantActivityRecord, Integer> {
	...
	@Modifying
	@Transactional
	@Query(value = "update  applicant_activity_record set status=:status , percentage_score=:percentage_score, count=:count , points_earned=:points_earned where  record_id =:record_id ",nativeQuery = true)
